    ...
    :fDetector(Det)
{
	...
	fAddDetectionSystemRCMPCmd = new G4UIcmdWithADoubleAndUnit("/DetSys/det/addRCMP",this);
	fAddDetectionSystemRCMPCmd->SetGuidance("Add Detection System RCMP with specified beam opening");
	fAddDetectionSystemRCMPCmd->AvailableForStates(G4State_PreInit,G4State_Idle);
...
{	
	...
	delete fAddDetectionSystemRCMPCmd;
...
void DetectorMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
	...
	if(command == fAddDetectionSystemRCMPCmd) {
		fDetector->AddDetectionSystemRCMP(fAddDetectionSystemRCMPCmd->GetNewDoubleValue(newValue));
	}
